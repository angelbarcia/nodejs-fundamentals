...
const getEmployees = (pageNo = 0, pageSize = 25) => {
  ...
  return EmployeeModel.find({}, { photo: false }, { skip, limit });
};

const hireEmployee = (newEmployee) => {
  // This function has been updated.
  const { identity, ...newEmployeeWithoutIdentity } = newEmployee;
  const employeeToBeAdded = { _id: identity, ...newEmployeeWithoutIdentity };

  const employee = new EmployeeModel(employeeToBeAdded);
